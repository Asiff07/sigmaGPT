                    ...
                    <div className='flex flex-wrap max-sm:justify-center gap-5'>
                        {images.map((item,index)=>(
                            <a key={index} target='_blank' className='relative group block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-shadow duration-300' href={item.imageUrl}>
                                ...
                                <p className='absolute bottom-0 right-0 text-xs bg-black/50 backdrop-blur text-white px-4 rounded-tl-xl opacity-0 group-hover:opacity-100 transition duration-300'>Created By: {item.userName}</p>
                            </a>
                        ))}
                    </div>
                )
